    ...
    override fun getCurrentCutShortHeight(view: View): Int {
        ...
        return if (!isCusShortVisible(context)) {
            ...
            if (Build.VERSION.SDK_INT >= 23 && view != null && view.rootWindowInsets != null) {
                ...
                val top =
                    cls.getDeclaredMethod("getSafeInsetTop").invoke(displayCutoutInstance) as Int
                val bottom =
                    cls.getDeclaredMethod("getSafeInsetBottom").invoke(displayCutoutInstance) as Int
                val left =
                    cls.getDeclaredMethod("getSafeInsetLeft").invoke(displayCutoutInstance) as Int
                val right =
                    cls.getDeclaredMethod("getSafeInsetRight").invoke(displayCutoutInstance) as Int
                safeInsets[left, top, right] = bottom
                boundingRects.addAll(
                    cls.getDeclaredMethod("getBoundingRects")
                        .invoke(displayCutoutInstance) as MutableList<Rect>
                )
                if (top != 0) {
                    return top
                }
